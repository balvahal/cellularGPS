function centroids = cellularGPS_centroids2table(annotation)
trackedCellIds = annotation.getTrackedCellIds;
centroids = zeros(length(annotation) * length(trackedCellIds), 5);
counter = 1;
for t= 1:length(annotation.singleCells)
...
centroids = array2table(centroids, 'VariableNames', {'cell_id', 'centroid_col', 'centroid_row', 'timepoint', 'value'});
end