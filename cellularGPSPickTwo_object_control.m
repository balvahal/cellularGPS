classdef cellularGPSPickTwo_object_control < handle
    %% Properties
    %   ___                       _   _
    ...
    methods
        ...
        %
        %
        function obj = cellularGPSPickTwo_object_control(pkTwo)
            %%%
            % parse the input
            q = inputParser;
            ...
            obj.imag3 = imread(fullfile(pkTwo.moviePath,'RAW_DATA',pkTwo.smda_databaseSubset.filename{pkTwo.indImage}));
            ...
            infoBk_textMessage = uicontrol('Parent',infoBk_panelMessage,'Style','text','Units','characters','String','Happy Viewing!',...
            ...
            infoBk_editStepSize = uicontrol('Parent',infoBk_panelInfo,'Style','edit','Units','characters',...
                'FontSize',14,'FontName','Verdana',...
                'String',num2str(1),...
                'Position',[40, 0.5, 15,2.6923],...
                'Callback',{@obj.infoBk_editStepSize_Callback});
            
            infoBk_textStepSize = uicontrol('Parent',infoBk_panelInfo,'Style','text','Units','characters','String','step size',...
                'FontSize',10,'FontName','Verdana','HorizontalAlignment','left',...
                'Position',[57, 0.5, 20, 2.6923]);
            ...
            tabGPS_tableSettings = uitable('Parent',tabGPS,'Units','characters',...
                ...
                'ColumnEditable',logical([0,0,0]),...
                'ColumnFormat',{obj.pkTwo.ity.channel_names(1),'numeric','numeric'},...
                'ColumnWidth',{'auto' 'auto' 'auto'},...
                'FontSize',8,'FontName','Verdana',...
            ...
            handles.infoBk_editStepSize = infoBk_editStepSize;
            handles.infoBk_textStepSize = infoBk_textStepSize;
        ...
        function obj = tabContrast_findImageHistogram(obj)
            [obj.contrastHistogram,~] = histcounts(reshape(obj.pkTwo.gui_imageViewer.imag3,1,[]),-0.5:1:255.5);
        end
        %%
        %
        ...
        function obj = tabContrast_newColormapFromContrastHistogram(obj)
            ...
            obj.pkTwo.gui_imageViewer.gui_main.Colormap = cmap;
        end
        ...
        function obj = infoBk_editTimepoint_Callback(obj,~,~)
            ...
            elseif indImage > height(obj.pkTwo.smda_databaseSubset)
                obj.pkTwo.indImage = height(obj.pkTwo.smda_databaseSubset);
            else
                obj.pkTwo.indImage = indImage;
            end
            obj.pkTwo.gui_imageViewer.loop_stepX;
            handles.infoBk_editTimepoint.String = num2str(obj.pkTwo.indImage);
            guidata(obj.gui_main,handles);
        end
        %%
        %
        function obj = infoBk_editStepSize_Callback(obj,~,~)
            handles = guidata(obj.gui_main);
            indImage = str2double(handles.infoBk_editStepSize.String);
            indImage = round(indImage);
            if indImage < 1
                obj.pkTwo.indImage = 1;
            elseif indImage > height(obj.pkTwo.smda_databaseSubset)
                obj.pkTwo.indImage = height(obj.pkTwo.smda_databaseSubset)-1;
            else
                obj.pkTwo.indImage = indImage;
            end
            obj.pkTwo.stepSize = indImage;
            handles.infoBk_editStepSize.String = num2str(obj.pkTwo.indImage);
            guidata(obj.gui_main,handles);
        end
        %%
        %
        function obj = tabGPS_tableGroup_CellSelectionCallback(obj,~,eventdata)
            ...
            if isempty(eventdata.Indices)
                ...
                if any(obj.pkTwo.pointerGroup > obj.pkTwo.ity.number_group)
                    % move pointer to last entry
                    obj.pkTwo.pointerGroup = obj.pkTwo.ity.number_group;
                end
                return
            else
                obj.pkTwo.pointerGroup = sort(unique(eventdata.Indices(:,1)));
            end
            
            myGroupOrder = obj.pkTwo.ity.order_group;
            gInd = myGroupOrder(obj.pkTwo.pointerGroup(1));
            if any(obj.pkTwo.pointerPosition > obj.pkTwo.ity.number_position(gInd))
                % move pointer to first entry
                obj.pkTwo.pointerPosition = 1;
            end
        ...
        function obj = tabGPS_tablePosition_CellSelectionCallback(obj,~,eventdata)
            ...
            if isempty(eventdata.Indices)
                ...
                if any(obj.pkTwo.pointerPosition > obj.pkTwo.ity.number_position(gInd))
                    % move pointer to last entry
                    obj.pkTwo.pointerPosition = obj.pkTwo.ity.number_position(gInd);
                end
                return
            else
                obj.pkTwo.pointerPosition = sort(unique(eventdata.Indices(:,1)));
            end
            obj.tabGPS_loop;
            obj.pkTwo.gui_imageViewer.loop_stepX;
        end
        ...
        function obj = tabGPS_tableSettings_CellSelectionCallback(obj,~,eventdata)
            ...
            myGroupOrder = obj.pkTwo.ity.order_group;
            gInd = myGroupOrder(obj.pkTwo.pointerGroup(1));
            pInd = obj.pkTwo.ity.ind_position{gInd};
            pInd = pInd(1);
            if isempty(eventdata.Indices)
                % if nothing is selected, which triggers after deleting data,
                % make sure the pointer is still valid
                if any(obj.pkTwo.pointerSettings > obj.pkTwo.ity.number_settings{pInd})
                    % move pointer to last entry
                    obj.pkTwo.pointerSettings = obj.pkTwo.ity.number_settings(pInd);
                end
                return
            else
                obj.pkTwo.pointerSettings = sort(unique(eventdata.Indices(:,1)));
            end
            obj.tabGPS_loop;
            obj.pkTwo.gui_imageViewer.loop_stepX;
        end
        ...
        function obj = tabGPS_loop(obj)
            ...
            tableGroupData = cell(obj.pkTwo.ity.number_group,...
                length(get(handles.tabGPS_tableGroup,'ColumnName')));
            n=0;
            for i = obj.pkTwo.ity.order_group
                n = n + 1;
                tableGroupData{n,1} = obj.pkTwo.ity.group_label{i};
                tableGroupData{n,2} = i;
                tableGroupData{n,3} = obj.pkTwo.ity.number_position(i);
            end
            ...
            myGroupOrder = obj.pkTwo.ity.order_group;
            gInd = myGroupOrder(obj.pkTwo.pointerGroup(1));
            myPositionOrder = obj.pkTwo.ity.order_position{gInd};
            tablePositionData = cell(length(myPositionOrder),...
                length(get(handles.tabGPS_tablePosition,'ColumnName')));
            n=0;
            for i = myPositionOrder
                n = n + 1;
                tablePositionData{n,1} = obj.pkTwo.ity.position_label{i};
                tablePositionData{n,2} = i;
                tablePositionData{n,3} = obj.pkTwo.ity.position_xyz(i,1);
                tablePositionData{n,4} = obj.pkTwo.ity.position_xyz(i,2);
                tablePositionData{n,5} = obj.pkTwo.ity.position_xyz(i,3);
                tablePositionData{n,6} = obj.pkTwo.ity.number_settings(i);
            end
            ...
            pInd = obj.pkTwo.ity.ind_position{gInd};
            pInd = pInd(1);
            mySettingsOrder = obj.pkTwo.ity.order_settings{pInd};
            tableSettingsData = cell(length(mySettingsOrder),...
                length(get(handles.tabGPS_tableSettings,'ColumnName')));
            n=1;
            for i = mySettingsOrder
                tableSettingsData{n,1} = obj.pkTwo.ity.channel_names{obj.pkTwo.ity.settings_channel(i)};
                tableSettingsData{n,2} = obj.pkTwo.ity.settings_exposure(i);
            ...
            %
            myGroupOrder = obj.pkTwo.ity.order_group;
            obj.pkTwo.indG = myGroupOrder(obj.pkTwo.pointerGroup(1));
            myPositionOrder = obj.pkTwo.ity.ind_position{gInd};
            obj.pkTwo.indP = myPositionOrder(obj.pkTwo.pointerPosition(1));
            mySettingsOrder = obj.pkTwo.ity.ind_settings{pInd};
            obj.pkTwo.indS = mySettingsOrder(obj.pkTwo.pointerSettings(1));
            obj.pkTwo.updateFilenameListImage;
            %%
            %
            handles.infoBk_textTimepoint.String = sprintf('of %d\ntimepoint(s)',height(obj.pkTwo.smda_databaseSubset));
            guidata(obj.gui_main,handles);
        end
        ...
        function obj = tabMakeCell_buttongroup_SelectionChangedFcn(obj,~,~)
            ...
            switch lower(handles.tabMakeCell_buttongroup.SelectedObject.String)
                ...
                case 'track 2 cell'
                    ...
                    handles.tabMakeCell_togglebuttonDelete.BackgroundColor = inactiveColor;
                    handles.tabMakeCell_togglebuttonMother.BackgroundColor = inactiveColor2;
                    handles.tabMakeCell_togglebuttonAddTrack2Cell.BackgroundColor = activeColor2;
            end
            guidata(obj.gui_main,handles);
        end
        %%
        ...
        function obj = tabMakeCell_loop(obj)
            ...
            %% Cell Table
            %
            existingCells = 1:length(obj.pkTwo.mcl.makecell_logical);
            existingCells = existingCells(obj.pkTwo.mcl.makecell_logical);
            makeCellData = cell(length(obj.pkTwo.mcl.makecell_logical),...
                length(handles.tabMakeCell_table.ColumnName));
            n=0;
            for i = existingCells
                n = n + 1;
                makeCellData{n,1} = i;
                makeCellData{n,2} = num2str(obj.pkTwo.mcl.makecell_ind{i});
                makeCellData{n,3} = obj.pkTwo.mcl.makecell_mother(i);
            end
        ...
        function obj = tabMakeCell_table_CellSelectionCallback(obj,~,eventdata)
            if isempty(eventdata.Indices)
                % if nothing is selected, which triggers after deleting data,
                % make sure the pointer is still valid
                obj.pkTwo.mcl.find_pointer_next_makecell;
                return
            else
                handles = guidata(obj.gui_main);
                obj.pkTwo.mcl.pointer_makecell3 = handles.tabMakeCell_table.Data{eventdata.Indices(1,1),1};
                if isempty(obj.pkTwo.mcl.pointer_makecell3)
                    obj.pkTwo.mcl.pointer_makecell3 = obj.pkTwo.mcl.pointer_next_makecell;
                end
                if ~isempty(obj.pkTwo.mcl.makecell_ind{obj.pkTwo.mcl.pointer_makecell3})
                    obj.pkTwo.mcl.pointer_track2 = obj.pkTwo.mcl.pointer_track;
                    obj.pkTwo.mcl.pointer_track = obj.pkTwo.mcl.makecell_ind{obj.pkTwo.mcl.pointer_makecell3}(1);
                    obj.pkTwo.gui_imageViewer.highlightTrack;
                end
            end
        end
        %%
        ...
        function obj = menuViewTracks_Callback(obj,~,~)
            ...
            if obj.menu_viewTrackBool
                ...
                for i = 1:length(obj.pkTwo.gui_imageViewer.trackCircle)
                    obj.pkTwo.gui_imageViewer.trackCircle{i}.Visible = 'off';
                    obj.pkTwo.gui_imageViewer.trackLine{i}.Visible = 'off';
                    obj.pkTwo.gui_imageViewer.trackText{i}.Visible = 'off';
                end
            else
        ...
        function obj = tabMakeCell_pushbuttonNewCell_Callback(obj,~,~)
            obj.pkTwo.mcl.newCell;
            obj.tabMakeCell_loop;
        end
        %%
        %
        function obj = tabMakeCell_pushbuttonAddTrack2Cell_Callback(obj,~,~)
            obj.pkTwo.mcl.addTrack2Cell;
            obj.tabMakeCell_loop;
            obj.pkTwo.gui_imageViewer.updateTrackText;
        end
        %%
        %
